...
const LearningPathVisualization: React.FC<LearningPathProps> = ({
  ...
  const getStatusIcon = (status: string) => {
    switch(status) {
      ...
      case 'current':
        return <RadioButtonUnchecked className="w-6 h-6 text-blue-500 animate-pulse" />;
      case 'locked':
        return <Lock className="w-6 h-6 text-gray-400" />;
      default:
        return <RadioButtonUnchecked className="w-6 h-6 text-gray-400" />;
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-yellow-50 to-orange-50 rounded-2xl shadow-lg p-6">
            <div className="flex items-center gap-3 mb-3">
              <Psychology className="w-8 h-8 text-orange-500" />
